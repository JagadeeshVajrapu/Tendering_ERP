...
import { DynamicMasterDatasetPanel } from '@/components/debug/DynamicMasterDatasetPanel';
...
function DynamicMasterDatasetContent() {
  ...
  const { data, isLoading, isFetching, isSuccess, error, refetch } = useQuery({
    queryKey: ['dynamic-master-dataset', activeDocumentId, refresh],
    queryFn: () => api.getDynamicMasterDataset(token!, activeDocumentId, refresh),
    ...
    <DashboardLayout>
      <div className="mb-6">
        ...
        <h1 className="flex items-center gap-2 text-2xl font-bold">
          <Database className="h-7 w-7 text-indigo-700" />
          Dynamic Master Dataset
        </h1>
        <p className="mt-1 text-sm text-muted-foreground">
          Single source of truth — core and dynamic parameters. NIT Analysis reads only from this dataset.
        </p>
      </div>
      ...
      <div className="mb-8 flex flex-col gap-3 rounded-xl border bg-white p-4 shadow-sm sm:flex-row sm:items-end">
        ...
        <Button onClick={load} disabled={!documentIdInput.trim()}>Build dataset</Button>
        {activeDocumentId && (
      ...
      {isLoading && activeDocumentId && (
        <div className="flex flex-col items-center py-20 text-muted-foreground">
          ...
          Building dynamic master dataset…
        </div>
      )}

      {error && activeDocumentId && !isLoading && (
        <div className="rounded-xl border border-red-200 bg-red-50 p-6 text-center text-sm text-red-700">
          {getErrorMessage(error, 'Failed to build master dataset')}
        </div>
      )}

      {payload && <DynamicMasterDatasetPanel data={payload} />}
    </DashboardLayout>
  );
}

export default function DynamicMasterDatasetPage() {
  return (
    <Suspense fallback={<div className="flex min-h-[40vh] items-center justify-center"><Loader2 className="h-8 w-8 animate-spin text-indigo-600" /></div>}>
      <DynamicMasterDatasetContent />
    </Suspense>
  );
}
