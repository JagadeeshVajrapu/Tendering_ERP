...
import { AiParameterCandidatePanel } from '@/components/debug/AiParameterCandidatePanel';
...
function ParameterCandidatesContent() {
  ...
  const { data, isLoading, isFetching, isSuccess, error, refetch } = useQuery({
    queryKey: ['parameter-candidates', activeDocumentId, refresh],
    queryFn: () => api.getParameterCandidates(token!, activeDocumentId, refresh),
    ...
    <DashboardLayout>
      <div className="mb-6">
        ...
        <h1 className="flex items-center gap-2 text-2xl font-bold">
          ...
          AI Parameter Candidates + Alias Mapping
        </h1>
        <p className="mt-1 text-sm text-muted-foreground">
          Step 1 AI extraction with canonical parameter names. Original tender labels are preserved in{' '}
          <strong>Original Label</strong> (e.g. Earnest Money Deposit → EMD Amount).
        </p>
      </div>
      ...
      <div className="mb-8 flex flex-col gap-3 rounded-xl border bg-white p-4 shadow-sm sm:flex-row sm:items-end">
        ...
        <Button onClick={load} disabled={!documentIdInput.trim()}>Extract candidates</Button>
        {activeDocumentId && (
      ...
      {!activeDocumentId && (
        <div className="rounded-xl border border-dashed py-16 text-center text-muted-foreground">
          Enter a document ID and click Extract candidates.
        </div>
      )}
      ...
      {isLoading && activeDocumentId && (
        <div className="flex flex-col items-center py-20 text-muted-foreground">
          ...
          Running AI extraction + alias mapping…
        </div>
      )}

      {error && activeDocumentId && !isLoading && (
        <div className="rounded-xl border border-red-200 bg-red-50 p-6 text-center text-sm text-red-700">
          {getErrorMessage(error, 'Failed to extract parameter candidates')}
        </div>
      )}

      {payload && <AiParameterCandidatePanel data={payload} />}
    </DashboardLayout>
  );
}

export default function ParameterCandidatesPage() {
  return (
    <Suspense fallback={<div className="flex min-h-[40vh] items-center justify-center"><Loader2 className="h-8 w-8 animate-spin text-violet-600" /></div>}>
      <ParameterCandidatesContent />
    </Suspense>
  );
}
