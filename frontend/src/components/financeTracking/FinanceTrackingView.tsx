...
function requirementBadge(status: FinancialRequirementStatus) {
  const map: Record<FinancialRequirementStatus, string> = {
    not_required: 'bg-slate-100 text-slate-600',
    pending: 'bg-red-100 text-red-800',
    partial: 'bg-amber-100 text-amber-900',
    complete: 'bg-emerald-100 text-emerald-800',
    overpaid: 'bg-blue-100 text-blue-800',
    informational: 'bg-indigo-100 text-indigo-800',
  };
  return map[status];
}

function requirementLabel(status: FinancialRequirementStatus) {
  const map: Record<FinancialRequirementStatus, string> = {
...
export function FinanceTrackingView({ token, tenderId, canEdit }: Props) {
  ...
  const summaryCards = useMemo(() => {
    ...
    const ctx = dashboard.financialContext;
    return [
      { label: 'Bid Value (NIT)', value: ctx.estimatedBidValue, subtitle: 'Estimated tender value' },
      { label: 'EMD Required', value: ctx.requiredEmd, subtitle: `Paid: ${formatCurrency(s.emdAmount)}` },
      { label: 'Tender Fee', value: ctx.tenderFee, subtitle: `Paid: ${formatCurrency(s.tenderFeeAmount)}` },
      { label: 'Total Committed', value: s.totalCommitted, subtitle: 'All instruments tracked' },
      { label: 'Remaining Due', value: ctx.totalRemaining, subtitle: 'Required − paid', highlight: ctx.totalRemaining > 0 },
      { label: 'PBG Tracked', value: s.pbgAmount, subtitle: ctx.performanceSecurityDisplay?.slice(0, 40) },
      { label: 'Pending Refund', value: s.pendingRefundAmount, highlight: s.pendingRefundAmount > 0 },
    ];
  }, [dashboard]);

  const suggestedAmount = (recordType: FinanceRecordType): number => {
    if (!dashboard?.financialContext) return 0;
    const ctx = dashboard.financialContext;
    switch (recordType) {
      case 'EMD':
        return ctx.requiredEmd || 0;
      case 'TENDER_FEE':
        return ctx.tenderFee || 0;
      case 'PBG':
        return ctx.requirements.find((r) => r.recordType === 'PBG')?.requiredAmount || 0;
      default:
        return 0;
    }
  };

  const openCreate = (presetType?: FinanceRecordType) => {
    setEditingRecord(null);
    const recordType = presetType || 'EMD';
    const amount = suggestedAmount(recordType);
    setForm({ ...EMPTY_FORM, recordType, amount });
    setShowForm(true);
  };
  ...
  return (
    <div className="space-y-6">
      ...
      {/* NIT requirements vs tracked */}
      <section className="rounded-xl border border-indigo-100 bg-gradient-to-br from-indigo-50/80 to-white p-5 shadow-sm">
        <div className="mb-4 flex flex-wrap items-start justify-between gap-3">
          <div>
            <h2 className="text-lg font-semibold text-slate-900">Tender Financial Requirements</h2>
            <p className="text-xs text-muted-foreground">
              {dashboard.financialContext.source === 'nit_analysis'
                ? 'Values from NIT analysis — compared with your tracked payments'
                : dashboard.financialContext.source === 'tender_record'
                  ? 'Partial data from tender record — complete NIT analysis for full requirements'
                  : 'Complete NIT analysis to auto-fill bid value, EMD, and tender fee'}
            </p>
          </div>
        ...
        {dashboard.financialContext.bidSubmissionDeadline && (
          <p className="mb-4 rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm text-slate-700">
            <span className="font-medium">Bid submission deadline:</span>{' '}
            {dashboard.financialContext.bidSubmissionDeadline}
          </p>
        )}
          ...
          <Table>
            <TableHeader>
              <TableRow>
                ...
                <TableHead>Required (NIT)</TableHead>
                <TableHead>Tracked / Paid</TableHead>
            ...
            <TableBody>
              {dashboard.financialContext.requirements.map((req: TenderFinancialRequirement) => (
                <TableRow key={req.label}>
                  <TableCell className="font-medium">{req.label}</TableCell>
                  <TableCell>
                    {req.status === 'informational'
                      ? req.requiredDisplay || formatCurrency(req.requiredAmount)
                      : req.requiredDisplay || formatCurrency(req.requiredAmount) || '—'}
                  </TableCell>
                  <TableCell>{req.status === 'informational' ? '—' : formatCurrency(req.trackedAmount)}</TableCell>
                  <TableCell>
                    {req.status === 'informational' ? '—' : formatCurrency(req.remainingAmount)}
                  </TableCell>
                  <TableCell>
                    <Badge className={requirementBadge(req.status)}>{requirementLabel(req.status)}</Badge>
                  </TableCell>
                  {canEdit && (
                    <TableCell className="text-right">
                      {req.recordType && req.status !== 'informational' && req.status !== 'complete' && (
                        <Button variant="outline" size="sm" onClick={() => openCreate(req.recordType)}>
      ...
      <section className="rounded-xl border bg-white p-5 shadow-sm">
        <h2 className="mb-4 text-lg font-semibold text-slate-900">Finance Summary</h2>
